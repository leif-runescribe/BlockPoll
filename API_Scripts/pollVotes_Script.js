...
const main = async () => {
    ...
    let getPollVotesFuncTxData = ElectionInterface.encodeFunctionData("getGeneral_PollVotes", ["0x534c4353"]);
    const getPollVotesFuncTx = {
        to: ElectionAddress,
        nonce: nonce,
        gasPrice: gasPrice,
        gasLimit: gasLimit,
        data: getPollVotesFuncTxData,
        chainId: 80001
    }
    let getPollVotesFuncSentTx = await adminWallet.sendTransaction(getPollVotesFuncTx);
    await getPollVotesFuncSentTx.wait(1);
    console.log(getPollVotesFuncSentTx, "\n");
    res.tx = getPollVotesFuncSentTx
