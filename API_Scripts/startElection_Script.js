...
const main = async () => {
    ...
    let addStartFuncTxData = ElectionInterface.encodeFunctionData("startElection")
    const addStartFuncTx = {
        to: ElectionAddress,
        nonce: nonce,
        gasPrice: gasPrice,
        gasLimit: gasLimit,
        data: addStartFuncTxData,
        chainId: 80001
    }
    let addStartFuncSentTx = await adminWallet.sendTransaction(addStartFuncTx);
    await addStartFuncSentTx.wait(1);
    console.log(addStartFuncSentTx, "\n");
    res.tx = addStartFuncSentTx
