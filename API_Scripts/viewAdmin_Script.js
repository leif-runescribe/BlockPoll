...
const main = async () => {
    ...
    let addViewAdminFuncTxData = PreElectionInterface.encodeFunctionData("viewAdmins")
    const addViewAdminFuncTx = {
        to: PreElectionAddress,
        nonce: nonce,
        gasPrice: gasPrice,
        gasLimit: gasLimit,
        data: addViewAdminFuncTxData,
        chainId: 80001
    }
    let addViewAdminFuncSentTx = await adminWallet.sendTransaction(addViewAdminFuncTx);
    await addViewAdminFuncSentTx.wait(1);
    // console.log(addViewAdminFuncSentTx, "\n");
    res.tx = addViewAdminFuncSentTx
